...
public:
  ...
  {
    enabled_objects_[std::string(obj_str)] = level;
  }
...
private:
  ...
  bool IsTraceEnabled(const std::string &obj_str, TraceLevel level) const;

  std::map<const std::string, TraceLevel> enabled_objects_;
};
